...
Before recording an account request for a group with [approve](/staff-docs/scripts/approve/), check to see if the group already has an account. Be
sure to use different forms and abbreviations of the group's name to maximize
the chance that a group will be found.

This script will search [LDAP](/staff-docs/infrastructure/ldap/) for case-insensitive
